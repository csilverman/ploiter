...
// print_r($matches[0]);
//
// So yeah, this works. Any tag markup wrapped in {{ and }} is optional, so
// it'll be removed if the tag wasn't populated.
...
// so $optional_tags is now an array where each item is a {{XXX}} template item
  ...
  foreach ( $array as $key => $value ) {
    // first thing, populate all the template tags that we have values for.

    $tmp_key = '[[' . $key . ']]';
    $markup = str_replace( $tmp_key, $value, $markup );
  }

  // next, clean up any remaining optional tags: ones that
  // weren't populated.

  // do this by cycling through the array of optional tags:

  foreach ( $optional_tags as $unpopulated_tag ) {
    // and replacing all of these with emptnesss

    $markup = str_replace( $unpopulated_tag, '', $markup );
  }
...
// Comparison function
// This is used for sorting all the items according to year.
